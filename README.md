...
本仓库为 [JASMINE145-ACT](https://github.com/JASMINE145-ACT) 基于 Claude Code Best V5 的定制版本，针对中文用户和特定工作场景进行了调整。

### 定制内容

| 目录/文件 | 说明 |
|-----------|------|
| `src/components/WorkflowProgress.tsx` | 工作流进度可视化组件 |
| `mcp_servers/claude-runner/` | Claude Runner MCP 服务 |
| `mcp_servers/google_maps/` | Google Maps API 集成 |
| `docs/superpowers/` | 高级功能文档和开发计划 |
| `start-ccb.ps1` | Windows 快速启动脚本 |

### 环境要求

- **Bun** >= 1.3.11（必需）
- **Node.js** >= 18（备用）
- **Python** >= 3.10（仅 MCP servers 使用）
...
本项目配置**完全独立**于官方 Claude Code：
- 配置文件在项目目录下，不影响官方版本
- 不会共享或覆盖官方 Claude Code 的配置
- 可以在同一机器上同时使用官方版和本定制版

### 从官方版迁移

如果你之前使用过官方 Claude Code，本项目可以无缝迁移：

| 项目 | 说明 |
|------|------|
| API Key | 相同，配置到 `.env` 即可 |
| MCP 配置 | 需要重新配置 `.mcp.json` |
| 会话历史 | 不共享，独立管理 |
| Skills | 不共享，需重新安装 |
| Feature Flags | 环境变量方式，独立生效 |

如需同时使用两个版本，建议通过不同别名调用。
...
| `GOOGLE_MAPS_API_KEY` | Google Maps API 密钥 | [ Google Cloud Console](https://console.cloud.google.com/) |
| `GOOGLE_API_KEY` | Google API 密钥 | [ Google Cloud Console](https://console.cloud.google.com/) |

### 使用注意

- `.env` 和 `.mcp.json` 为本地配置，**不包含在仓库中**，请自行配置
- API Key 等敏感信息需要自行申请和配置
- 如需使用 MCP 功能，请参考 `mcp_servers/` 下的说明文档
